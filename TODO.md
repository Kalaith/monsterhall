...
- `ContractStatus::Completed`/`Failed`/`Declined` are never assigned; resolved contracts are just dropped from `active_contracts`, and the contract desk has labels/colors for states that can never appear. Investigated: the tidy fix is to set the terminal status instead of dropping, and let the *next* day's `resolve_contracts` purge it — the `Completed | Failed | Declined => {}` arm already does exactly that, so the contract stays on the desk for the turn the player actually reads it. The obstacle is that `refresh_contracts` runs later in the same `resolve_day` (`resolution.rs:452`) and its opening `retain` purges terminal contracts before the player sees them. Moving that purge means `active_request_limit` and `next_sequence` (`guest.rs:69-71`) and the id seed at `depth.rs:545` must count only `Pending | Accepted`, or contract generation shifts.
